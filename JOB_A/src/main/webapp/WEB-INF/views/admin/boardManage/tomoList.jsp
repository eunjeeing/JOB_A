								...
								<div class="card shadow">
									<div class="card-body">
										...
										<table class="table table-hover"
											...
											<tbody>
												<c:forEach items="${tomoList }" var="tomo">
													<tr align="center" class="trtr">
														<td>${tomo.board_No}</td>
														<td class="goBoard" id="${tomo.board_No }">${tomo.board_Title}</td>
														<td>${tomo.mem_Nick}</td>
														<td>${tomo.board_Date}</td>
														<td>${tomo.board_View}</td>
														<td><c:if test="${tomo.board_Status eq 'Y'}">
																<span class='badge badge-success'>정상</span>
															</c:if> <c:if test="${tomo.board_Status eq 'B'}">
																<span class='badge badge-danger'>블라인드</span>
															</c:if>  <c:if test="${tomo.board_Status eq 'N'}">
																<span class='badge badge-secondary'>삭제</span>
															</c:if> </td>
														<td>
																<c:if test="${tomo.board_Status eq 'Y' }">
																	<button class="btn mb-2 btn-light" style="margin-bottom:0 !important;"
																		onclick="location.href='${pageContext.request.contextPath}/admin/updateStatusB.do?board_No=${tomo.board_No}&type_No=${tomo.type_No }'">블라인드</button>
																</c:if>
																<c:if test="${tomo.board_Status eq 'B' }">
																	<button class="btn mb-2 btn-light" style="margin-bottom:0 !important;"
																		onclick="location.href='${pageContext.request.contextPath}/admin/updateStatusY.do?board_No=${tomo.board_No}&type_No=${tomo.type_No }'">활성화</button>
																</c:if>
																<c:if test="${tomo.board_Status eq 'N'}">
