											...
											<tbody>
												<c:forEach items="${qnaList }" var="qna">
													<tr align="center" class="trtr">
														...
														<td><c:if test="${qna.board_Status eq 'Y'}">
																<span class='badge badge-success'>정상</span>
															</c:if> <c:if test="${qna.board_Status eq 'B'}">
																<span class='badge badge-danger'>블라인드</span>
															</c:if>  <c:if test="${qna.board_Status eq 'N'}">
																<span class='badge badge-secondary'>삭제</span>
															</c:if> </td>
														<td>
																<c:if test="${qna.board_Status eq 'Y' }">
																	<button class="btn mb-2 btn-light" style="margin-bottom:0 !important;"
																		onclick="location.href='${pageContext.request.contextPath}/admin/updateStatusB.do?board_No=${qna.board_No}&type_No=${qna.type_No }'">블라인드</button>
																</c:if>
																<c:if test="${qna.board_Status eq 'B' }">
																	<button class="btn mb-2 btn-light" style="margin-bottom:0 !important;"
																		onclick="location.href='${pageContext.request.contextPath}/admin/updateStatusY.do?board_No=${qna.board_No}&type_No=${qna.type_No }'">활성화</button>
																</c:if>
																<c:if test="${qna.board_Status eq 'N'}">
...
<script>
	...
	$(function(){
		$(".goBoard").on("click", function(){
			...
			location.href = "${pageContext.request.contextPath}/board2/selectQnAList.do?board_No="+ board_No;
		});
	});
