											...
											<tbody>
												<c:forEach items="${tomoCommentList }" var="tomo">
													<tr align="center" class="trtr">
														...
														<td><c:if test="${tomo.comm_Status eq 'Y'}">
																<span class='badge badge-success'>정상</span>
															</c:if> <c:if test="${tomo.comm_Status eq 'B'}">
																<span class='badge badge-danger'>블라인드</span>
															</c:if>  <c:if test="${tomo.comm_Status eq 'N'}">
																<span class='badge badge-secondary'>삭제</span>
															</c:if> </td>
														<td>
																<c:if test="${tomo.comm_Status eq 'Y' }">
																	<button class="btn mb-2 btn-light" style="margin-bottom:0 !important;"
																		onclick="location.href='${pageContext.request.contextPath}/admin/updateCommStatusB.do?comm_No=${tomo.comm_No}&type_No=${tomo.type_No }'">블라인드</button>
																</c:if>
																<c:if test="${tomo.comm_Status eq 'B' }">
																	<button class="btn mb-2 btn-light" style="margin-bottom:0 !important;"
																		onclick="location.href='${pageContext.request.contextPath}/admin/updateCommStatusY.do?comm_No=${tomo.comm_No}&type_No=${tomo.type_No }'">활성화</button>
																</c:if>
																<c:if test="${tomo.comm_Status eq 'N'}">
