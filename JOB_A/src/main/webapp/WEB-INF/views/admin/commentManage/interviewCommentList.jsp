								...
								<div class="card shadow">
									<div class="card-body">
										...
										<table class="table table-hover"
											...
											<tbody>
												<c:forEach items="${interviewCommentList }" var="inter">
													<tr align="center" class="trtr">
														<td>${inter.comm_No}</td>
														<td class="goBoard" id="${inter.board_No }">${inter.comm_Content}</td>
														<td>${inter.mem_Nick}</td>
														<td><fmt:parseDate var="parsedDate" value="${inter.comm_Date}" pattern="yyyy-MM-dd HH:mm:ss.S"/>
														<fmt:formatDate value="${parsedDate}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
														<td><c:if test="${inter.comm_Status eq 'Y'}">
																<span class='badge badge-success'>정상</span>
															</c:if> <c:if test="${inter.comm_Status eq 'B'}">
																<span class='badge badge-danger'>블라인드</span>
															</c:if>  <c:if test="${inter.comm_Status eq 'N'}">
																<span class='badge badge-secondary'>삭제</span>
															</c:if> </td>
														<td>
																<c:if test="${inter.comm_Status eq 'Y' }">
																	<button class="btn mb-2 btn-light" style="margin-bottom:0 !important;"
																		onclick="location.href='${pageContext.request.contextPath}/admin/updateCommStatusB.do?comm_No=${inter.comm_No}&type_No=${inter.type_No }'">블라인드</button>
																</c:if>
																<c:if test="${inter.comm_Status eq 'B' }">
																	<button class="btn mb-2 btn-light" style="margin-bottom:0 !important;"
																		onclick="location.href='${pageContext.request.contextPath}/admin/updateCommStatusY.do?comm_No=${inter.comm_No}&type_No=${inter.type_No }'">활성화</button>
																</c:if>
																<c:if test="${inter.comm_Status eq 'N'}">
